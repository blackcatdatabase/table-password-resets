...
namespace BlackCat\Database\Packages\PasswordResets\Service;
 ...
 */
final class PasswordResetsAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PasswordResetRepository $passwordResetRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'password_resets' => $this->passwordResetRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\PasswordResets\ModuleException("Record {$idStr} not found for locking.");
            }
